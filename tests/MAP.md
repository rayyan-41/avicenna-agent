...
| `test_normalise_wiring.py` | 180 | Tests for the normaliser wiring in `AssemblyStage`: the stage emits `MarkdownNormalised` on every run; a messy weaver's output is cleaned; the written note has no horizontal rules adjacent to headings; the frontmatter block is preserved through normalisation; and word count is not materially changed (< 5% drift). |

...
| `test_write_back.py` | 374 | Tests for `_write_back` and `_unwrap_model_output` in pipeline/stages.py. The pipeline owns the frontmatter, not the model. These tests verify that `_write_back` preserves the on-disk frontmatter, unwraps chatty model output, and rejects truncation — exactly the guarantees that keep a note vault-safe. Includes tests for malformed frontmatter blocks. |
<!-- map:files:end -->

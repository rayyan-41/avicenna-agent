...
| `test_structure.py` | 517 | Tests for `avicenna.pipeline.structure`: idempotence (f(f(x)) == f(x)), fenced code blocks byte-identical, frontmatter untouched, TOC-anchor property (every link resolves to a heading with punctuation, colons, ampersands, em-dashes, non-ASCII, and duplicates), duplicate heading disambiguation, stray # demoted not deleted, restated heading stripped, pre-existing TOC not duplicated, positional numbering, and diagnostics. |

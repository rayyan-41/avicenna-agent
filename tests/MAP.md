...
| `test_stages_fixes.py` | 465 | Tests for the three stage defects fixed together: `_canonical_domain_dir` resolves a domain against the folder actually on disk (case-insensitive, hyphen/space tolerant) and only falls back to Title Case for a new domain, so the note folder and the MOC filename can no longer disagree; the tagging floor constructs a minimal valid array from the taxonomy when the tagger fails three times, rather than shipping `tags: []` and orphaning the note; and LinkingStage skips the model only when `get_related_notes` actually ran and returned nothing, never when the tool is merely absent. |

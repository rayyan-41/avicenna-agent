...
| `protocol.ts` | 159 | The wire contract — mirrors `avicenna/events.py` and `avicenna/bridge/server.py`. Declares `EventName` (the union of all event names), `EventFrame`, `Frame`, the `STAGES` list, and every typed result interface (`HelloResult`, `VaultInfo`, `RunHandle`, etc.). `PROTOCOL_VERSION` is checked at handshake; a mismatch rejects startup with a clear message. `App.onEvent`'s `never` check depends on `EventName` being a union, not a string. |

...
| `app.py` | 384 | Typer application, all top-level commands, subcommand group registration, TUI/headless launch helpers. |
...
| `avicenna keys` | `keys_cmd` | Shows the API key pool: size, source (env / file / single), fingerprints, grouped by provider. Reports the configured provider; `--all` shows every provider in the pool file. Read-only; never prints key material. |

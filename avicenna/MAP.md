...
| `auth.py` | 146 | Provider onboarding: validate a key with one real completion, persist it, report status. Deferred imports from `providers/` keep the auth module importable without pulling in the SDK. |
...
| `concurrency.py` | 30 | `gather_sections`: bounded concurrent execution of N awaitables via a semaphore (default from `settings.MAX_CONCURRENCY_DEFAULT`). One failure does not cancel siblings. |
...
| `settings.py` | 223 | Minimal settings resolver for `words_per_heading`, timeouts and `max_concurrency`. Precedence chain: CLI flag > env var > vault `.agents/config.json` > built-in default. Per-template overrides live in vault scope. Concurrency is clamped to `[1, 16]`. |
<!-- map:files:end -->

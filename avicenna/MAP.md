...
| `keypool.py` | 201 | API key pool for spreading parallel completions across multiple keys. Round-robin rotation with quarantine for definitively-bad keys (401/403). Precedence: MISTRAL_API_KEYS env var, ~/.avicenna/api_keys_pool file, single key from secrets. A pool of one is the normal case. |

...
| `auth.py` | 147 | Provider onboarding: validate a key with one real completion, persist it, report status. Deferred imports from `providers/` keep the auth module importable without pulling in the SDK. |
...
| `keypool.py` | 158 | API key pool for spreading parallel completions across multiple keys. Round-robin rotation with quarantine for definitively-bad keys (401/403). Precedence: MISTRAL_API_KEYS env var, ~/.avicenna/api_keys_pool file, single key from secrets. A pool of one is the normal case. |

...
| `powershell.py` | 113 | `PowerShellTool` executes vault `.ps1` scripts. `normalise_ps_value` handles PowerShell's `-File` parser quirk: bare tokens containing commas or spaces are interpreted as arrays, so values are wrapped in literal double quotes. This module is why CI runs the Python job on `windows-latest`. Stdin is `DEVNULL` because a vault script that reads stdin would consume bridge NDJSON frames. |

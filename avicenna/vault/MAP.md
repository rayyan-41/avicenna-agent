...
| `registry.py` | 388 | Theme and type registry. Accumulates inferred themes and types in taxonomy.json so the harness maintains a map of the reader's mind. Normalises case, separator and plural variants; mints new entries after a deterministic check; persists atomically. Provides a seam (`semantic_guard`) for the embedding-based similarity check that will later strengthen deduplication. |

...
| File | Loc | Role |
| --- | --- | --- |
| `__init__.py` | 104 | Public surface: re-exports neutral types, error classes, `EmbedTask`, `EmbeddingProvider`, and both fake providers eagerly. MistralProvider and GoogleEmbeddingProvider are behind PEP 562 `__getattr__`. Registers `mistral` and `fake` LLM factories plus `google` and `fake` embedding factories at import time (both real factories defer their module imports to construction). |

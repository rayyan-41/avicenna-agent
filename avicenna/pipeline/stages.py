...
class TocStage(PipelineStage):
    # DEAD CODE: removed from build_stages().  The TOC is now generated by
    # FormatterStage.apply_structure after numbering, so every anchor
    # resolves.  A TOC generated before numbering produced invented suffixes
    # for duplicate headings — anchors that existed nowhere in the document.
    # The class is kept temporarily for reference; the "toc" id is preserved
    # in the Stage literal for backwards compatibility with run logs.
    name: Stage = "toc"
    id = "toc"

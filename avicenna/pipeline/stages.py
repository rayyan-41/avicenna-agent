...
class TaggingStage(PipelineStage):
    ...
    async def run(self, ctx: RunContext) -> None:
        ...
        for attempt in range(1, 4):
            ...
            resolved_line = await _resolve_tags_against_registry(
                assembled_line, ctx, entities=effective_entities,
            )
...
async def _resolve_tags_against_registry(
    ...
    *,
    ...
    pass through unchanged.  *entities* must be supplied by the caller: entity
    tags are an open vocabulary of proper nouns, so unlike domains, categories
    and markers there is no closed set to recognise them by, and a name the
    caller does not declare is indistinguishable from a proposed theme.
    ...
    # Everything here passes through resolution untouched.  This set was once

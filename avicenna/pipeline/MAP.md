...
| `stages.py` | 1936 | All 13 stage implementations plus `build_stages()`. Contains the write-back guard (`_write_back`), the frontmatter pipeline, the atomic note writer, wikilink resolution for weaver output, and every vault-tool call with its graceful-degradation fallback. The bulk of pipeline logic lives here. |

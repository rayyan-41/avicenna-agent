...
| `context.py` | 95 | `RunSpec` (frozen inputs: topic, vault, provider, run flags) and `RunContext` (mutable state accumulated by stages). Keeping them separate means a stage can only corrupt the mutable half. |
...
| `linking.py` | 396 | Pure linking logic: the subject index that says which entity tags have a note of their own, first-mention wikilink insertion, and the Related Notes section rendered from `get_related_notes.ps1` output. The link policy itself is the vault script's, not reimplemented here. No model call anywhere — the linker that asked a model where links belonged invented targets that did not exist, and nothing here can invent one: every link names a filename read off disk. |
| `normalise.py` | 219 | Post-assembly Markdown normalisation: collapses consecutive horizontal rules, removes rules adjacent to headings, caps blank lines, ensures heading spacing. Applied on every write path so every note is cleaned regardless of which agent produced it. |
| `preflight.py` | 122 | JSON-first pre-flight parser. Asks the agent for a fenced `json` block; falls back to regex over prose with a warning. Validates heading count (1–40), enforces template word floors, and derives filesystem-safe slugs. |
...
| `sections.py` | 192 | Parallel section fan-out. Builds one closure per heading through `gather_sections`. Each task calls `one_shot()` for fresh context, retries once on exception or empty output, and Python (never the model) writes `_tmp/[slug]_chunk_NN.md`. |
...
| `stages.py` | 2281 | All 14 stage implementations plus `build_stages()`. Contains the write-back guard (`_write_back`), the frontmatter pipeline, the atomic note writer, wikilink resolution for weaver output, and every vault-tool call with its graceful-degradation fallback. The bulk of pipeline logic lives here. |
...
| `tagging.py` | 439 | Pure tagging logic: parsing tagger replies (labelled slots and positional fallback), entity derivation from the topic string, and tag-array assembly. No I/O, no RunContext, no provider. The model judges what a note is about; Python assembles the positional array. |

...
| `2026-09-07-terminal-interface-design.md` | 182 | Why the interface is designed before it is built: deciding what it must show tells the backend what to emit, and an event is cheapest to add while the stage that would emit it is open. Names the six events the design needs and the pipeline does not yet produce — routing's runner-up and margin, a typed link-resolution result, the near-miss on a minted theme, registry totals, resume availability, and mechanical tag assignment. Argues the registry view is product rather than chrome, since a map of the reader's attention that nobody sees is only a database; solves the concurrency problem by making the declared outline the progress display rather than a scrolling log; separates transient progress from the decisions worth reading after a run; and defers the aesthetic, which should be settled by looking rather than by reading a document. |
| `2026-09-07-vault-sovereignty-and-emergent-taxonomy-design.md` | 261 | The harness restating what the vault already knows, three times in one day: a MOC glob that could not match the vault's filenames, a closed tag enumeration validating an open entity vocabulary, and two disagreeing domain casings that renamed a file with months of history. Establishes that the vault owns structure — domains are root folders, categories are the folders inside them, the frontmatter schema is detected from existing notes — and the harness owns only generation and cohesion. Inverts taxonomy.json from a declared input into an accumulated registry of inferred themes and types, a map of where the reader's attention has gone, with embedding similarity guarding against the synonym drift that would otherwise turn that map into noise. Specifies the knowledge graph that replaces tag-overlap link scoring, and retires harness-maintained MOCs. |
| `2026-09-09-conversational-terminal-interface-design.md` | 1676 | Sections 1 to 4, of which only section 1 is approved. Reverses the 2026-09-07 spec on its one load-bearing input decision: there is a chat surface, and a run begins from inside it. Records why that is not a licence to build an assistant — `chat.py` already confines a conversation to five read-only tools in the core, so chat can look at the vault and never change it — and why plan approval is completing a stub the bridge already concedes rather than reversing the protocol's direction. Settles the runtime (Bun and OpenTUI, with the finding that Zig is not required and that Cline's docs are stale on the point), the full rewrite of `tui/` and the two learned behaviours of the old bridge that must survive it, and the decision that intent detection is a pure Python module so no model decides whether the pipeline runs. Notes that three of the six events the earlier spec demanded still do not exist, and that the pipeline has not been run end to end since the heading-length and linking changes. |
<!-- map:files:end -->
